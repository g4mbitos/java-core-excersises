...
public class AccountAnalytics {
    ...
    public Optional<Account> findRichestPerson() {
        return Optional.of(accounts.stream()
                .max(Comparator.comparing(acc -> acc.getBalance()))
                .orElseThrow(() -> new EntityNotFoundException("There is no the richest account")));
    }

    /**
     * Returns a {@link java.util.List} of {@link Account} that have a birthday month equal to provided.
     *
     ...
     * email domain like "gmail.com". And the value is a {@link java.util.List} of {@link Account} objects with a specific email domain.
     *
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        acc -> acc.getEmail().substring(acc.getEmail().indexOf('@') + 1),
                        HashMap::new,
                        Collectors.toCollection(ArrayList::new)));
    }
    ...
    /**
     * Returns a {@link java.util.List} of {@link Account} objects sorted by first and last names.
     *
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(acc -> acc.getEmail().substring(acc.getEmail().indexOf('@') + 1).equals(emailDomain));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .chars()
                .boxed()
                .collect(Collectors.groupingBy(i -> (char)(int)i, Collectors.counting()));
    }
